...
__v2.5.1__:
 ...
 - Version 2.5 is essentially a version 2.4 patch that was quickly distributed to avoid problems.  Version 2.5.1 (on GitHub) goes back to `sarima` from `astsa` version 2.3 and introduces a separate script, &#127381;  `ts.diag` &#127381;  that can be used for generic residual diagnostics.  It's basically an improved version of `stats::tsdiag`.

-  Update `tsplot` so it takes the argument `axes=FALSE` (before you had to use `xaxt=FALSE` and `yaxt=FALSE` instead)
