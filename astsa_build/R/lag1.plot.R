...
function(series, max.lag=1, corr=TRUE, smooth=TRUE, col=gray(.1), 
 ...
 for(h in 1:max.lag){
  tsplot(stats::lag(as.ts(series),-h), as.ts(series), type='p',  xy.labels=FALSE, xy.lines=FALSE,
          xlab=paste(name1,h,")",sep=""), ylab=name2, col=col, cex=cex, ...) 
   if (smooth) 
   lines(stats::lowess(ts.intersect(stats::lag(as.ts(series),-h), as.ts(series))[,1],
                ts.intersect(stats::lag(as.ts(series),-h), as.ts(series))[,2]), col=lwc, lwd=lwl)
   if (corr)
