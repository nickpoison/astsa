matrixpwr <- function(A, power) {
  ...
  tol <- 1e-8 * max(1, max(abs(Re(R))))  # tolerance for Im residue
  
# If A was real (not complex) but eigen decomposition introduced tiny
  # imaginary residues from floating-point roundoff, drop them.
  if (is.numeric(A) && is.complex(R)) {
    max_im <- max(abs(Im(R)))
    if (max_im < tol) {
      R <- Re(R)
    } else {
      warning("Result has non-negligible imaginary part (max |Im| = ",
              signif(max_im, 3), "); returning complex matrix.")
    }
  }
  # Snap near-zero real entries to exactly zero (roundoff from eigen decomposition)
  if (is.numeric(R)) {
    R[abs(R) < tol] <- 0
  }
  dimnames(R) <- dn
