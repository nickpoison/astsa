...
Xp = ts(as.vector(ks$xp), start=strt, freq=frq) + phat[2]
Pp = ts(as.vector(ks$Pp), start=strt, freq=frq)
Xs = ts(as.vector(ks$xs), start=strt, freq=frq) + phat[2]
Ps = ts(as.vector(ks$Ps), start=strt, freq=frq)
estimate = est$par 
u = cbind(estimate, SE) 
if (fixphi){rownames(u)=c("alpha", "sigw", "sigv"); est$par[2:3] = abs(est$par[2:3]) 
 } else { rownames(u)=c("phi", "alpha", "sigw", "sigv"); est$par[3:4] = abs(est$par[3:4])
 }
print(u)
